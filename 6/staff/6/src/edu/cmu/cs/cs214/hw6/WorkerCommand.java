 ...
 * (1) set the command's socket (by calling {@link #setSocket(Socket)}) for the
 * command to use to communicate results back to the client server, and (2)
 * execute the command by calling its {@link #run()} method.
 *
 * Refer to recitation 13 for an example of how this class should be used.
 */
