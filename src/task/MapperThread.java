...
/**
 * This class extends a thread and it starts a map task
 * 
 * @author yuruiz
 *
 */
public class MapperThread extends Thread {
	...
	public void run() {
		...
		for (int i = 0; i < filepartitions.size(); i++) {
			...
			/*
			 * If file not located locally, try to fetch files from other node
			 * that might have it
			 */
			if (!file.exists()) {
				...
				try {
					FileTransmission
							.askforfile(fileName, p.getOwners(), worker);
			...
			FileReader reader = new FileReader(Config.DataDirectory + "/"
					+ fileName);
	...
	private void shuffle(List<KeyValuePair> outputs) {
		...
		System.out.println("Start distribution records, record number "
				+ outputs.size());
