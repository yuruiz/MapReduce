...
public class Worker {
	...
	public void start() {
		...
		/*
		 * Initialize the File system by scanning the local files and build the
		 * filelist
		 */
		try {
			DFSbootstrap();
		...
		try {
			...
			while ((socket = listenSocket.accept()) != null) {
				...
				/*
				 * Processing the File fetching request from reduce task on
				 * other nodes
				 */
				case FILE_FETCH:
					...
					/*
					 * Check the file list to see if there is map result file
					 * for the Reduce task
					 */
				...
				case FILE_REQ:
					/*
					 * Processing file fetch request from map task on other
					 * nodes
					 */
	...
	private void register() {
		while (true) {
			try {
				...
				// if master is not on, simply try again
				continue;
			}
		}
	}
	...
	/*
	 * File system initialization, scan the local files and add them to the file
	 * list
	 */
